...
public class VeiculoDao {
    ...
    private static final String SELECT_ALL_MARCAS = "SELECT DISTINCT nomeMarca FROM Veiculo ORDER BY nomeMarca";
    private static final String SELECT_MODELOS_BY_MARCA = "SELECT DISTINCT nomeMod FROM Veiculo WHERE nomeMarca = ? ORDER BY nomeMod";
    private static final String SELECT_BY_MATRICULA = "SELECT matricula, nomeMarca, nomeMod, cor, potencia, combustivel FROM Veiculo WHERE matricula = ?";
    ...
    public List<String> getAllMarcas() {
        ...
        try (Connection conn = Db.getConn();
                PreparedStatement ps = conn.prepareStatement(SELECT_ALL_MARCAS);
                ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
    ...
    public List<String> getModelosByMarca(String nomeMarca) {
        List<String> modelos = new ArrayList<>();
        try (Connection conn = Db.getConn(); PreparedStatement ps = conn.prepareStatement(SELECT_MODELOS_BY_MARCA)) {
    ...
    public Veiculo getByMatricula(String matricula) {
        try (Connection conn = Db.getConn(); PreparedStatement ps = conn.prepareStatement(SELECT_BY_MATRICULA)) {
            ps.setString(1, matricula);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    Veiculo veiculo = new Veiculo();
